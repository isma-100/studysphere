...
// Global DM context — real-time delivery, unread counts, instant optimistic updates
...
export function DMProvider({ children }) {
  ...
  // ── Load all past DMs for this user ──
  ...
  // ── Subscribe to incoming DMs where I am the receiver ──
    ...
    const ch = supabase
      ...
      .on('postgres_changes', {
        ...
        setConversations(prev => {
          ...
          // Avoid duplicates in case optimistic message is still pending
  ...
  // ── Send a DM — optimistic update so it shows INSTANTLY ──
  async function sendDM(channelId, receiverId, content) {
    if (!content?.trim()) return

    // 1. Build a temporary message and show it RIGHT NOW
    ...
    // 3. Replace temp message with the real confirmed message from DB
    setConversations(prev => {
      const conv = prev[channelId]
      if (!conv) return prev
      if (error) {
        // Remove the temp message if insert failed — show nothing rather than a ghost
        return { ...prev, [channelId]: { ...conv, messages: conv.messages.filter(m => m.id !== tempId) } }
      }
      return {
        ...prev,
        [channelId]: {
          ...conv,
          messages: conv.messages.map(m => m.id === tempId ? data : m)
        }
      }
    })
  }

  // ── Get or create a conversation entry in state ──
